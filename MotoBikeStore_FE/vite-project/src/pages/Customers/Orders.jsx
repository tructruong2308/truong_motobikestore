...
/** ===== API BASE (chỉ 1 biến) =====
 * Vercel: VITE_API_BASE = https://truong-motobikestore-3.onrender.com/api
 * Local dev: rơi về http://127.0.0.1:8000/api
 */
const API_BASE = (
...
.ordersX .overlay { position: fixed; inset: 0; z-index: 99999; background: rgba(2,6,23,.65); display:flex; align-items:center; justify-content:center; animation: overlayIn .18s ease-out; }
.ordersX .overlayCard{ width:min(680px,92vw); border-radius:18px; padding:18px; background: linear-gradient(180deg, #0b1220, #0c1426); border:1px solid rgba(148,163,184,.25); box-shadow:0 30px 80px rgba(0,0,0,.45); animation: cardIn .22s ease-out; }
.ordersX .overlayHead{ display:flex; align-items:center; gap:10px; margin-bottom:10px; color:#e5e7eb }
.ordersX .overlayHead .badge{ font-size:12px; font-weight:900; padding:4px 10px; border-radius:999px; white-space:nowrap; background:rgba(99,102,241,.18); border:1px solid rgba(99,102,241,.38); color:#c7d2fe; }
...
.ordersX .overlayBtn{ height:38px; padding:0 14px; border-radius:10px; border:1px solid var(--line); background:#0f172a; color:#e2e8f0; font-weight:900; cursor:pointer }
.ordersX .overlayBtn.primary{ background:#14532d; border-color:#14532d }
@keyframes flashPulse { 0% { box-shadow: 0 0 0 0 rgba(96,165,250,.45); } 100% { box-shadow: 0 0 0 14px rgba(96,165,250,0); } }
  ...
  return (
    <div className="ordersX">
        ...
        <div className="overlay" onClick={() => setOverlay((o) => ({ ...o, open: false }))}>
          <div className="overlayCard" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="overlayActions">
              <button className="overlayBtn" onClick={() => setOverlay((o) => ({ ...o, open: false }))}>Đóng</button>
              <button className="overlayBtn primary" onClick={() => { setOverlay((o) => ({ ...o, open: false })); if (overlay.orderId) focusOrder(overlay.orderId); }}>Xem đơn</button>
            </div>
          </div>
        </div>
      )}
        ...
        <div className="noticeWrap">
          <div className="noticeHead">
            ...
            <div className="noticeActions">
              <button className="xbtn" onClick={() => { setNotices([]); localStorage.removeItem("order_notices"); }}>Đánh dấu đã đọc</button>
            </div>
          </div>
          <div className="noticeList">
            {notices.map((n) => (
              <div className="noticeItem" key={n.id} onClick={() => n.orderId && focusOrder(n.orderId)}>
                <div>
                  <div style={{ fontWeight: 800 }}>{n.text}</div>
                  <div style={{ opacity: .75, fontSize: 12 }}>{new Date(n.at).toLocaleString()}</div>
                </div>
                <button className="xbtn" onClick={(e) => { e.stopPropagation(); setNotices((prev)=>{const next=prev.filter(x=>x.id!==n.id); localStorage.setItem("order_notices", JSON.stringify(next)); return next;}); }}>X</button>
              </div>
            ))}
          </div>
        </div>
      ...
      <div className="toolbar">
        <input placeholder="🔍 Tìm theo mã đơn hoặc tên sản phẩm…" value={q} onChange={(e) => setQ(e.target.value)} style={{ minWidth: 280 }} />
      ...
      {loading ? (
        ...
        filtered.map((o) => {
          ...
          return (
            <div key={o.id} id={`order-${o.id}`} className={`card ${Number(highlightId) === Number(o.id) ? "highlight" : ""}`} style={{ marginBottom: 14 }}>
              <div className="head">
              ...
              <div className="foot">
                ...
                <div className="actions">
                  <button className="btn" onClick={() => addManyToCart(o)} disabled={!ds.length} title="Thêm toàn bộ sản phẩm của đơn vào giỏ">Mua lại</button>
                  <button className="btn danger" onClick={() => cancelOrder(o)} disabled={!canCancel} title="Hủy đơn (nếu API cho phép)">Hủy đơn</button>
                </div>
              </div>
            </div>
          );
